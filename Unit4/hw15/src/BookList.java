...
public class BookList {
    ...
    public HashMap<String, HashMap<String, Book>> getBooks() {
        return books;
    }
    ...
    public void align() {
        for (HashMap<String, Book> m : books.values()) {
            for (Book book : m.values()) {
                if (book.getCnt() < 3) {
                    book.addCnt(3 - book.getCnt());
                }
            }
        }
    }
    ...
    public boolean hasBook(Book book) {
        return books.get(book.getType()).containsKey(book.getId());
    }
    ...
    public void lost(String book) {
        books.get(book.substring(0, 1)).get(book.substring(2)).lost();
    }
