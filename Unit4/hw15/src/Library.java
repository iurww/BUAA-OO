...
public class Library {
    ...
    public void returnBook(String date, String stuId, String book) {
        ...
        if (checkDate(date, students.get(stuId).getBook(book))) {
            borrowAdmin.punish(date, stuId);
        }
        if (borrowBook.getType().equals("B")) {
    ...
    public boolean checkDate(String date, Book book) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        LocalDate borrowDate = LocalDate.parse(book.getBorrowDate(), formatter);
        LocalDate returnDate = LocalDate.parse(date, formatter);
        long l = ChronoUnit.DAYS.between(borrowDate, returnDate);
        if (book.getType().equals("B") && l > 30) {
            return true;
        } else if (book.getType().equals("C") && l > 60) {
            return true;
        }
        return false;
    }
